...
out_path <- "/Users/jendawk/Dropbox (MIT)/C Diff Recurrence Paper/Analyses/univariate_analysis_drug/16s/deseq2/"

files_x <- list.files(path="inputs/DEseq2_drug", pattern="counts*", full.names=TRUE, recursive=FALSE)
files_y <- list.files(path="inputs/DEseq2_drug", pattern="col*", full.names=TRUE, recursive=FALSE)
